...
def out_of_scope_node(state: DisputeState) -> dict:
    ...
    return {
        ...
        "messages": [AIMessage(content="🛡️ **Bloqueio de Escopo:** Sou o SentinelOps, assistente para delivery. Não posso responder sobre outros assuntos.")]
    }
...
def hydrate_and_check_cache_node(state: DisputeState) -> dict:
    ...
    # 🚀 CÁLCULO SEGURO E DETERMINÍSTICO DO TOTAL DO RECIBO
    ...
    cache_key = semantic_cache.build_cache_key(ticket_id, masked_query, telemetry, trust_text)
    ...
    try:
        # Passando as variáveis numéricas estritas para o Cofre Matemático
        cached = validate_egress(
            cached,
            dispute_amount=amount,
            receipt_total_amount=receipt_total_amount,
        )
    except EgressValidationError as error:
        print(f"  🛡️ [Semantic Cache] HIT rejeitado pelo egress validator: {error}")
        return {}
    ...
    return {
        ...
        "human_in_the_loop_required": cached["recommended_action"] == "escalar_humano",
...
def route_security(state: DisputeState) -> str:
    ...
    if state.get("recommended_action") in ("bloqueio_seguranca", "escalar_humano"):
...
def route_after_triage(state: DisputeState) -> str:
    ...
    if db_risk == "high":
        if amount > medium_min:
            print(f" >> 🛡️ PERFIL HIGH + DISPUTA ACIMA DE R$ {medium_min:.2f}: Escalando diretamente para HITL.")
            return "human_review"
...
def build_graph():
    ...
    workflow.add_conditional_edges("hydrate_and_check_cache", route_after_cache, {"investigator": "investigator", END: END})
    workflow.add_conditional_edges("investigator", route_investigator, {"tools": "tools", END: END})
